...
class NFCTag:
    ...
    async def get_info(self):
        await self.get_uid()
        await self.get_manufacturer()
        await self.get_block(1)
        await self.get_block(2)
    ...
    def check_tag(self):
        if self._device:
            await self.get_uid()
            auth = await self._device.mifare_classic_authenticate_block(self.uid, 4)
            if auth:
    ...
    async def get_manufacturer(self):
        if self._device:
            data = await self._device.read_mifare_classic(0)
            if data:
    ...
    async def get_uid(self, ret=False):
        if self._device:
            uid = await self._device.read_passive_target()
            if uid:
                self._uid = uid
            if ret:
                return uid
    ...
    async def memorydump(self, debug=False, rtn=False):
        if self._device:
            if debug:
                print("Reading NFC Tag...")
            for block in range(self._nblocks):
                block_data = await self._device.read_mifare_classic(block, debug=debug)
                if debug:
                    print(
                        "[{:80}] {} %\r".format(
        ...
        **kwargs,
        ...
        if self._device and not cache:
            ...
            if uid:
                kwargs.pop("uid")
            if not checked_tag:
                checked_tag = await self._device.read_passive_target()
            if checked_tag:
                if on_detect:
                    on_detect(**kwargs)
                for block in range(self._bps):
                    ...
                    if skip_st and self.is_st(b_index):
                        continue
                    block_data = await self._device.read_mifare_classic(
    ...
    async def get_block(self, number, debug=False):
        if number > self._nblocks - 1:
            raise IndexError
        if self._device:
            block_data = await self._device.read_mifare_classic(number, debug=debug)
            if block_data and len(block_data) == self._blen:
                self._blocks[number][:] = block_data

        return self.block(number)

    async def find_wr_sectors(
        ...
        for bl in range(self._bps, self._nblocks, self._bps):
            ...
            if cache:
                ...
                nb = await self.get_block(bl, debug=debug)
            if sum(nb) != 0:
    ...
    async def read_nfc_records(self, n=None, debug=False, cache=False, **kwargs):
        # SECTORS :
        raw_records = {}
        ri = 0
        if not n:
            ...
            nfc_nonempty_sectors = await self.find_wr_sectors(
