...
class NFCTag:
    def __init__(self, memory_size=1024, uid=b'', device=None, init=True):
        ...
        self._blocks = {i: self._buffer_memory[i * self._nsectors: (i * self._nsectors) + self._nsectors]
                        for i in range(0, self._nblocks)}
        self._sectors = {i: [self._blocks[k] for k in range(i*self._bps, (i*self._bps)+self._bps)] for i in range(self._nsectors)}
    ...
    def sector(self, number):
        return {number*self._bps+i: bytes(self._sectors[number][i]) for i in range(self._bps)}
            ...
            if sector == 0:
                ...
                for block in self._sectors[sector]:
                    if block == self._sectors[sector][0]:
                        print('Block {}: {}'.format(n, _BB.format(hexlify(bytes(block), " ").decode())))
                    elif block == self._sectors[sector][1] or block == self._sectors[sector][2]:
                        print('Block {}: {}'.format(n, _BM.format(hexlify(bytes(block), " ").decode())))
                    else:
                        bl = hexlify(bytes(block), " ").decode()
                        if self._device and self._memdumped is True:
                            if self._device.auth_key == 'NFCS':
                                keyA = _BG.format(hexlify(self._device.mifare_keys['MAD'], " ").decode()) + ' '
                                keyB = ' ' + _BY.format(hexlify(self._device._auth_key, " ").decode())
                            else:
                ...
                for block in self._sectors[sector]:
                    if block == self._sectors[sector][-1]:
                        ...
                        if self._device and self._memdumped is True:
                            if self._device.auth_key == 'NFCS':
                                keyA = _BG.format(hexlify(self._device._auth_key, " ").decode()) + ' '
                                keyB = ' ' + _BY.format(hexlify(self._device._auth_key, " ").decode())
                            else:
                                keyA = _BG.format(bl[:_KLHXFM])
                                keyB = _BY.format(bl[-_KLHXFM:])
                        ...
                        print('Block {}: {}'.format(n, hexlify(bytes(block), " ").decode()))
                    n += 1
    ...
    def memorydump(self, debug=False, rtn=False):
        if self._device:
            ...
            for block in range(self._nblocks):
                ...
                if debug:
                    print('[{:80}] {} %\r'.format(int((block/self._nblocks)*80)*'#',
                                                  int((block/self._nblocks)*100)), end='')
                if block_data:
                    self._memory[block * self._blen: (block * self._blen) + self._blen] = block_data

            self.get_mad()
            if debug:
                print('[{:80}] {} %\r'.format(int(((block+1)/self._nblocks)*80)*'#',
    ...
    def get_sector(self, number, byblocks=False, debug=False, checked_tag=False,
                   on_detect=None, on_reading=None, at_end=None, **kargs):
        ...
        if self._device:
            ...
            if checked_tag:
                ...
                for block in range(self._bps):
                    ...
                    block_data = self._device.read_mifare_classic(b_index, debug=debug,
                                                                  uid=uid)
                    if block_data:
        ...
        if byblocks:
            ...
            sector = bytes()
            for block in self.sector(number).values():
                sector += block
            return sector
    ...
    def find_wr_sectors(self, nfcfilter=True, stop_at_first_empty=False, debug=False):
    ...
    def find_tlvblock(self, datablock, debug=False,):
    ...
    def read_nfc_records(self, n=None, debug=False,
                         **kargs):
        # SECTORS :
        raw_records = {}
        ri = 0
        if not n:
            ...
            nfc_nonempty_sectors = self.find_wr_sectors(stop_at_first_empty=True,
                                                        debug=debug)
            if debug:
        ...
        for nfcs in nfc_nonempty_sectors:
            tlv_block += self.find_tlvblock(self.get_sector(nfcs, debug=debug, **kargs))
        record = NDEFRecord(tlv_block)
        while True:
            try:
...
class MAD1:
    ...
    def __init__(self, madblocks=None):
        ...
        if madblocks:
            ...
            self._aids = dict(zip([k for k in range(1, 16)], [self._mad[i:i+2] for i in range(2, 32, 2)]))
        ...
        if self._mad:
            ...
            for sec_index, aid in self._aids.items():
                if sec_index in self._nfc_sectors:
                    print('AID{} : 0x{} [NFC SECTOR]'.format(sec_index, hexlify(aid).decode()))
                else:
